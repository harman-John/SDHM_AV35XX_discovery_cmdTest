...
public class FunctionTestSDHMOTA {
    private final static String TAG = FunctionTestSDHMOTA.class.getSimpleName();
    private static FunctionTestSDHMOTA instance;
    private LightX mLightX;

    public static FunctionTestSDHMOTA getInstance(){
        if (instance == null) {
            instance = new FunctionTestSDHMOTA();
        }
        return instance;
    }
    ...
    private LightX.Delegate delegate = new LightX.Delegate() {
        ...
        @Override
        public boolean lightXFirmwareReadStatus(LightX lightX, LightX.FirmwareRegion firmwareRegion, int i, byte[] bytes) {
            Log.d(TAG,"lightXFirmwareWriteStatus firmwareRegion= "
                    +firmwareRegion+",firmwareWriteOperation="+",i ="+ i+",bytes ="+ HexHelper.byteToArray(bytes));
            return false;
        }
    ...
    public void goFunction(int pos) {
        ...
        switch (pos) {
            ...
            case Constants.START_CALIBRATION_STATUS:
                mLightX.writeAppWithUInt32Argument(Command.App_0xB2, 0);
                break;
            ...
            case Constants.WRITE_APP_AUDIO_EQ_PRESET:
                mLightX.writeAppAudioEQPreset(AudioEQPreset.Music);
                break;
